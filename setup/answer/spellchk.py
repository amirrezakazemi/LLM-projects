...
def select_correction(typo, predict):
    new_predict = []
    for dic in predict:
        if dic['token_str'][0] in string.ascii_lowercase:
            dic['lev_dist'] = Levenshtein.distance(typo, dic['token_str'])
            new_predict.append(dic)
    min_dist = min(new_predict, key=lambda x: (x['lev_dist']))
    return min_dist['token_str']
